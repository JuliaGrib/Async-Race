...
class AppWin extends Component {
    ...
    async getWinners(){
        ...
        for (let i = 0; i < dataWinList.length; i++){
            ...
            const response3 = await fetch(`${baseUrl}/garage?id=${dataWinList[i]["wins"]}`);
            const elem = await response3.json();
            ...
            trElem.innerHTML = `
                ...
                <td>${elem[0]["name"]} ${elem[0]["model"] || ' '}</td>
